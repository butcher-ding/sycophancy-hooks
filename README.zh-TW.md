...
- **macOS 或 Linux**（需 POSIX `fcntl` 做檔案鎖——**不支援 Windows**）
- **Node.js 16+**
- **Python 3**（用於 `fcntl` 檔案鎖）
- **Claude Code 2.x**

## 安裝

```bash
git clone https://github.com/YOUR_GITHUB_USERNAME/sycophancy-hooks.git  # ← 換成你的 GitHub 帳號
cd sycophancy-hooks
chmod +x hooks/*.js

# 建立 symlink 到 Claude Code
...
3. **跑 `node -c hooks/*.js`** 驗證語法
4. **跑測試**：`node tests/parseMarkdownBias.test.js`（改 parser 邏輯時請補測試）
5. **在 macOS 或 Linux 測**——Windows 明確不支援
